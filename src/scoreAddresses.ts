...
const { RPC_URL, PRIVATE_KEY, SCORER_API_KEY, SCORER_ENDPOINT, COMMUNITY_ID, ATTESTER_CONTRACT_ADDRESS, SCHEMA_UID } =
  process.env;
...
const encodeScoreData = (
  ...
  // Convert stamps objects to arrays for tuple encoding
  const stampsAsArrays = stamps.map((stamp) => [stamp.provider, stamp.score]);

  return abiCoder.encode(
    ["bool", "uint8", "uint128", "uint32", "uint32", "tuple(string,uint256)[]"],
    [passing_score, score_decimals, scorer_id, score, threshold, stampsAsArrays]
  );
};
...
const createAttestationData = (address: string, scoreData: V2ScoreResponseData, scorerId: string) => {
  ...
  for (let i = 0; i < addresses.length; i += batchSize) {
    ...
    for (const address of batch) {
      try {
        ...
        const txHash = await writeScoreOnchain(contract, address, scoreData, scorerId);
...
const main = async () => {
  try {
    ...
    const contract = new ethers.Contract(ATTESTER_CONTRACT_ADDRESS, ATTESTER_ABI, wallet);
